...
//data
var street3='ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ',//52chars,only caps
street4='',
street5='AbCDEFGhijkLmnOpQRstUVwXyz ABCDEFGHIJKLMnOpQRstUVwXyz ',//52chars+2spaces-mixed
street7='AbCDEFGhijkLmnOpQRstUVwXyz',//26chars-mixed
street8='ABCDEFGHI JKLMNOPQRSTUVWXYZ';//26chars+1space-caps

describe('kyepix100 test scenario', function(){
    //this.timeout(10000);
    ...
    it('Test3. Fill street value with caps text>50 chars' ,async function(){
        await lib.setField(driver,noPage.noLoc.no_addressStreet,street3);
    })
    
    it('TestRes3. System displays error' ,async function(){
        title=await lib.isVisible1(driver,noPage.noLoc.no_streetAlert);
        await title.should.equal(true);
        title=await lib.getValueCss(driver,noPage.noLoc.no_streetAlert);
        await title.should.equal('max 50 χαρακτήρες');
        await driver.findElement(By.id(noPage.noLoc.no_addressStreet)).sendKeys(Key.CONTROL,'a');
        await driver.findElement(By.id(noPage.noLoc.no_addressStreet)).sendKeys(Key.CONTROL,'c');

        lib.erase(driver,noPage.noLoc.no_addressStreet);
    })
    it('Test4. Fill street value with no char' ,async function(){
        await lib.setField(driver,noPage.noLoc.no_addressStreet,street4);
    })
    ...
    it('Test5. Fill street value with mixed text >50chars' ,async function(){
        await lib.setField(driver,noPage.noLoc.no_addressStreet,street5);
    })
    ...
    it('Test6. Fill street value with copy-paste caps text>50 chars' ,async function(){
        await lib.clikId(driver,noPage.noLoc.no_addressStreet);
        await driver.findElement(By.id(noPage.noLoc.no_addressStreet)).sendKeys(Key.CONTROL,'v');
    })
    ...
    it('Test7. Fill street value with mixed text <50chars' ,async function(){
        await lib.setField(driver,noPage.noLoc.no_addressStreet,street7);
    })
    ...
    it('Test8. Fill street value with copy-paste caps text<50 chars' ,async function(){
        await lib.setField(driver,noPage.noLoc.no_addressStreet,street8);
    })
    it('TestRes8. System doesnt display street error' ,async function(){
        title=await lib.isVisible1(driver,noPage.noLoc.no_streetAlert);
        await title.should.equal(false);        
    })
