...
describe('kyepix81 test scenario', function(){
    //this.timeout(10000);
    ...
    it('step3. edit office details',async function(){
    ...
    it('Step4. Cancel edit',async function(){
    ...
    it('Step5. User confirms the cancelation',async function(){
    ...
    it('step6. go to office details',async function(){        
        await driver.sleep(sA);
        await ofs.findClick(driver,d.nameV2);
        tit= await lib.getValueCss(driver,ofs.ofLocs.o_offSubpageTitle);
        await tit.should.equal('Επισκόπηση στοιχείων γραφείου ΚΥΕΠΙΧ');
    ...
    it('step7. edit office details',async function(){
        await lib.clikCss(driver,ofs.ofLocs.o_offSubpageEditBtn);
        await driver.sleep(sA);
        tit= await lib.getValueCss(driver,ofs.ofLocs.o_offSubpageTitle);
        await tit.should.equal('Επεξεργασία στοιχείων Γραφείου ΚΥΕΠΙΧ');
    ...
    it('Step8. edit office',async function(){
        //await driver.sleep(sll);
    ...
    it('click edit button',async function(){
    ...
    it('9.click accept in the edit ',async function(){
        await lib.clikCss(driver,ofs.ofLocs.o_offSubpageAlertYes);
    })

    it('update data',async function(){
        tit=d.openHoursV;
        d.openHoursV=d.openHoursV2;
        d.openHoursV2=tit;
    })

    it('9.3.check the notification for the edit ',async function(){
        await driver.sleep(500);
        tit=await lib.getTextId(driver,ofs.ofLocs.o_snackbar);
        await tit.should.equal('Το γραφείο ΚΥΕΠΙΧ με Όνομα '+d.nameV2+' έχει ενημερωθεί επιτυχώς!');
    })

    it('9.2.check the page that the system redirected ',async function(){
        await driver.sleep(sA);
        tit=await lib.getValueCss(driver,ofs.ofLocs.o_title);
        await tit.should.equal('Διαχείριση Γραφείων ΚΥΕΠΙΧ');
    })

    it('find office',async function(){
        await ofs.findClick(driver,d.nameV2);
    })

    it('9.1.check the changed field (openHours)',async function(){
        await driver.sleep(sA);
        title= await lib.getTextXpath(driver,ofs.ofLocs.o_offProfileOpHours);
        await title.should.equal(d.openHoursV); 
    })

    it('Step10. Go to edit page',async function(){
        await lib.clikCss(driver,ofs.ofLocs.o_offSubpageEditBtn);
        await driver.sleep(sA);
        tit= await lib.getValueCss(driver,ofs.ofLocs.o_offSubpageTitle);
        await tit.should.equal('Επεξεργασία στοιχείων Γραφείου ΚΥΕΠΙΧ');
    ...
    it('Ste11. Click delete',async function(){
    ...
    it('Step12. Accept tge deletion of the office',async function(){
        await lib.clikCss(driver,ofs.ofLocs.o_offSubpageAlertYes);
        await driver.sleep(sA);
    })

    it('12.3.check the notification for the edit ',async function(){
        await driver.sleep(500);
        tit=await lib.getTextId(driver,ofs.ofLocs.o_snackbar);
        await tit.should.equal('Το γραφείο ΚΥΕΠΙΧ με Όνομα '+d.nameV2+' έχει διαγραφεί επιτυχώς!');
    })

    it('12.2.check the page that the system redirected ',async function(){
        await driver.sleep(sA);
        tit=await lib.getValueCss(driver,ofs.ofLocs.o_title);
        await tit.should.equal('Διαχείριση Γραφείων ΚΥΕΠΙΧ');
    })

    it('9.1.check that the office is deleted',async function(){
        await driver.sleep(sA);
        tit=await ofs.ofExists(driver,d.nameV2)
        await tit.should.equal(false);
    })
